...
void Array::display_array(int*& disp_array, size_t disp_size, const std::unordered_set<size_t>& highlight) {
    display_array(disp_array, disp_size, 0, highlight);
}

void Array::display_array(int*& disp_array, size_t disp_size, size_t cap, const std::unordered_set<size_t>& highlight) {
...
Array::Array(size_t initCapacity) : capacity(initCapacity), size(0) {
    try {
...
void Array::build(const std::vector<int>& values) {
    for (int val : values) {
        Array::append(val);
        Array::display();
    }
...
void Array::bubble_sort(int* buf) {
...
int* Array::merge_sort(int* buf, size_t l, size_t r) {
    ...
    if (r - l + 1 == size) {
        ...
        display_array(sorted, r - l + 1, r - l + 1);
    }
    return sorted;
}

void Array::insertion_sort(const int* buf) {
